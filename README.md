...
```python
@register("astrbot_plugin_haimeng_code", "久", "海梦酱码管理系统", "2.2.1")
...
### v2.2.1 - "数据迁移版" (2026-02-10)

**📤 用户导出（新功能）**
- 新增 `4-6` 命令导出全部注册用户，自动分批发送（每批50个），适配QQ消息长度限制
- 每批消息间隔0.5秒发送，避免QQ限流
- 导出格式为纯QQ号列表，可直接复制用于 `4-5` 批量导入实现数据迁移
- 新增 `jiu导出` 快捷命令

**📥 用户导入修复**
- 修复 `_import_users` 方法未定义的bug（4-5 命令无法使用）
- 导入时自动过滤非数字输入并提示跳过数量
- 用户管理菜单补充显示 4-5 和 4-6 选项

**� 管理员菜单导航系统**
- 新增 `D` 回退一级、`D2` 回退两级、`Q` 返回主菜单的层级导航
- 子菜单操作完成后保活在当前菜单，无需重新输入 `jiu`
- 输入状态（如导入用户）完成后自动回到上级菜单
- 新增 `STATE_PARENT` 层级映射和 `_get_menu_display` 菜单展示辅助方法

**�🔧 架构改进**
- `AdminHandler.handle()` 支持返回 `List[str]` 批量消息类型
- `main.py` 消息路由支持批量响应（逐条yield + 延迟发送）
- `DataManager` 新增 `get_all_registered_users()` 无限制导出接口
